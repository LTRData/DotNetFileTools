...
public static partial class VmQuery
{
    ...
    {
        public readonly nint BaseAddress;          // PVOID
        public readonly nint AllocationBase;       // PVOID
        public readonly uint AllocationProtect;    // DWORD
        public readonly nint RegionSize;           // SIZE_T
        public readonly uint State;                // DWORD
        public readonly uint Protect;              // DWORD
        public readonly uint Type;                 // DWORD
    }
    ...
    public readonly record struct Range(nint AllocationBase, nint Size, uint Type);
    ...
    /// </summary>
    public static bool TryGetAllocationRange(nint anyAddressInside, out Range range)
    {
        range = default;

        if (VirtualQuery(anyAddressInside, out var mbi0, (nuint)Unsafe.SizeOf<MEMORY_BASIC_INFORMATION>()) == 0)
        {
            return false;
        }
        ...
        for (; ; )
        {
            if (VirtualQuery(cursor, out var mbi, (nuint)Unsafe.SizeOf<MEMORY_BASIC_INFORMATION>()) == 0)
            {
                break;
            }
